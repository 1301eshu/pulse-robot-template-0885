import { useState, useEffect } from 'react';
import { Eye, Headphones, Play, Pause, Share2 } from 'lucide-react';
import { Button } from '@/components/ui/button';

interface BlogInteractionBarProps {
  content: string;
  title: string;
  url: string;
  views?: number;
  onViewIncrement?: () => void;
}

const BlogInteractionBar = ({ 
  content, 
  title, 
  url, 
  views = 0, 
  onViewIncrement 
}: BlogInteractionBarProps) => {
  const [isPlaying, setIsPlaying] = useState(false);
  const [synthesis, setSynthesis] = useState<SpeechSynthesis | null>(null);
  const [utterance, setUtterance] = useState<SpeechSynthesisUtterance | null>(null);
  // Use blog URL as unique key for localStorage
  const viewKey = `blog_views_${url}`;
  const getInitialViews = () => {
    const stored = localStorage.getItem(viewKey);
    if (stored) return parseInt(stored, 10);
    const randomViews = Math.floor(Math.random() * 1001) + 1000;
    localStorage.setItem(viewKey, randomViews.toString());
    return randomViews;
  };
  const [currentViews, setCurrentViews] = useState(getInitialViews);

  useEffect(() => {
    // Initialize speech synthesis
    if (typeof window !== 'undefined' && 'speechSynthesis' in window) {
      setSynthesis(window.speechSynthesis);
    }

  // Ensure views are persisted in localStorage
  localStorage.setItem(viewKey, currentViews.toString());
  }, [onViewIncrement]);

  const extractTextContent = (htmlContent: string): string => {
    // Remove HTML tags and get clean text
    const div = document.createElement('div');
    div.innerHTML = htmlContent;
    return div.textContent || div.innerText || '';
  };

  const handleListen = () => {
    if (!synthesis) return;

    if (isPlaying) {
      // Pause/Stop
      synthesis.cancel();
      setIsPlaying(false);
      setUtterance(null);
    } else {
      // Start playing
      const textContent = extractTextContent(content);
      const newUtterance = new SpeechSynthesisUtterance(textContent);
      
      newUtterance.rate = 0.9;
      newUtterance.pitch = 1;
      newUtterance.volume = 1;
      
      newUtterance.onstart = () => setIsPlaying(true);
      newUtterance.onend = () => {
        setIsPlaying(false);
        setUtterance(null);
      };
      newUtterance.onerror = () => {
        setIsPlaying(false);
        setUtterance(null);
      };

      setUtterance(newUtterance);
      synthesis.speak(newUtterance);
    }
  };

  const handleShare = (platform: 'facebook' | 'linkedin') => {
    const encodedUrl = encodeURIComponent(url);
    const encodedTitle = encodeURIComponent(title);
    
    let shareUrl = '';
    
    if (platform === 'facebook') {
      shareUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`;
    } else if (platform === 'linkedin') {
      shareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodedUrl}`;
    }
    
    window.open(shareUrl, '_blank', 'width=600,height=400');
  };

  return (
<div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between border-t border-gray-200 pt-4 text-sm gap-4">
  {/* Left: Views + Listen */}
  <div className="flex flex-col sm:flex-row sm:items-center gap-4 text-gray-600">
    {/* Views */}
    <div className="flex items-center gap-2">
      <Eye className="w-4 h-4 text-gray-900" />
      <span className="font-semibold text-gray-900">{currentViews.toLocaleString()} Views</span>
    </div>

    {/* Divider (hidden on mobile) */}
    <span className="hidden sm:block h-4 w-px bg-gray-300" />

    {/* Listen */}
    <div className="flex items-center gap-2">
      <Button
        variant="ghost"
        size="sm"
        onClick={handleListen}
        className="p-0 h-auto font-medium text-blue-600 hover:text-blue-700 hover:bg-transparent"
        disabled={!synthesis}
      >
        {isPlaying ? (
          <>
            <Pause className="w-4 h-4 mr-1" />
            Pause
          </>
        ) : (
          <>
            <Headphones className="w-4 h-4 mr-1" />
            Listen
          </>
        )}
      </Button>
    </div>
  </div>

  {/* Right: Share */}
  <div className="flex items-center gap-2">
    <span className="font-semibold underline text-gray-500 text-sm">Share this article :</span>
    <div className="flex gap-2">
      <button
        onClick={() => handleShare('linkedin')}
        className="w-8 h-8 rounded-full bg-[#0077B5] flex items-center justify-center hover:opacity-80 transition-opacity"
        aria-label="Share on LinkedIn"
      >
        <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
        </svg>
      </button>
      <button
        onClick={() => handleShare('facebook')}
        className="w-8 h-8 rounded-full bg-[#1877F2] flex items-center justify-center hover:opacity-80 transition-opacity"
        aria-label="Share on Facebook"
      >
        <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
        </svg>
      </button>
    </div>
  </div>
</div>

  );
};

export default BlogInteractionBar;