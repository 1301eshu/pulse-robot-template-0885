...
export default function EnterpriseCapabilities({
  ...
  // ✅ mark once client mounted
  useEffect(() => {
    hasMounted.current = true;
  }, []);

  // ✅ only run client-side scroll + auto-rotate
  useEffect(() => {
  ...
  return (
    <section className="bg-[#0F1117] text-white py-20 overflow-hidden mt-20">
      <div className="max-w-7xl mx-auto px-6">
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <span
                      className="absolute bottom-0 left-0 h-[2px] rounded-full bg-blue-500 animate-grow-3s"
                      style={{ width: "100%" }} // ✅ fixed SSR mismatch (not 0%)
                    />
                  )}
                </button>
              );
        ...
        {/* ---- Content Area ---- */}
        <div className="grid lg:grid-cols-2 gap-10 items-center animate-fade-slide-up">
          {currentFeature && (
            <>
              {/* ---- Left Column ---- */}
              <div className="space-y-6">
